...
/* freqwatch */
#define CONF_FILE   "freqwatch.conf"
MYSQL *con;
char db_mon_table[32];
char fw_db_ip[32];
char fw_db_user[32];
char fw_db_pass[32];
char fw_db_db[32];
	...
	while (!do_exit) {
		if (s->lrmix) {
			...
			for(j=0; j < b0->len; j++) {
                ...
                /* freqwatch */
                insertdb(b0->buf+j, 2, 1);
                insertdb(b1->buf+j, 2, 1);
                /* end freqwatch */
			}
...
int main(int argc, char **argv)
{
    ...
    if(conff != NULL){
        ...
        while(fgets(line, sizeof(line), conff) != NULL){
            tmp = strtok(line, "=");
            token = trimwhitespace(tmp);

            if(strcmp(token, "db_mon_table") == 0){
                token = strtok(NULL, "=");
                strncpy(db_mon_table, trimwhitespace(token), sizeof(db_mon_table));

            } else if(strcmp(token, "db_ip") == 0){
                token = strtok(NULL, "=");
                strncpy(fw_db_ip, trimwhitespace(token), sizeof(fw_db_ip));

            } else if(strcmp(token, "db_port") == 0){
                token = strtok(NULL, "=");
                fw_db_port = atoi(trimwhitespace(token));

            } else if(strcmp(token, "db_db") == 0){
                token = strtok(NULL, "=");
                strncpy(fw_db_db, trimwhitespace(token), sizeof(fw_db_db));

            } else if(strcmp(token, "db_user") == 0){
                token = strtok(NULL, "=");
                strncpy(fw_db_user, trimwhitespace(token), sizeof(fw_db_user));
    ...
    if(mysql_real_connect(con, fw_db_ip, fw_db_user, fw_db_pass, fw_db_db, fw_db_port, NULL, 0) == NULL){
        fprintf(stderr, "%s\n", mysql_error(con));
        mysql_close(con);
        exit(1);
    }
